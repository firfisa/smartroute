...
| --- | --- | --- | --- | --- | --- |
| `cmd/smartroute` | CLI | experimental | Command parsing, config validation, synthetic decision trace, Guard/engine lifecycles, observation controls/reporting, durable shadow-writer lifecycle, and evidence status/backup/verify/restore | Durable policy application, destructive evidence cleanup, static-baseline inference, and active Clash configuration | `cmd/smartroute/main_test.go`; data plane exercised by Test Labs |
...
| `internal/sidecar` | Data plane | experimental | SOCKS admission, TLS first-flight orchestration, stage enforcement, relay, decision/diagnostic/post-commit-outcome events, and cancellation-time handler draining | Payload inspection, application-success inference, SQLite I/O, durable policy evaluation, post-commit replay, or active Clash modification | Sidecar net.Pipe lifecycle/outcome tests, Test Lab and Mihomo Lab |
...
| `internal/netrelay` | Data plane | experimental | Context-aware bidirectional TCP copying, directional count result, half-close handling, cancellation closure, and copy-goroutine join | Routing decisions, application-success inference, retry, replay, or payload inspection | `internal/netrelay/relay_test.go`; consumers in Guard, Sidecar and Test Lab |
...
| `internal/observe` | Persistence | experimental | HMAC-pseudonymize typed events, stamp random trial scope, write bounded schema-2 JSONL, implement lifecycle controls, and aggregate identity-free readiness/post-commit relay metrics | Learned policy, payload capture, static-baseline/total-wire inference, application-success measurement, cloud upload, or active Clash access | Recorder/session/control/report/schema-migration tests; CLI sink/report tests |
...
| `Recorder.Record(event)` | `internal/observe` | Typed bounded event with optional raw target or relay counters | Pseudonymous schema-2 JSONL record or paused no-op | Oversized/write errors return to caller; runtime caller warns once and continues routing | experimental | Hashing, relay privacy, pause, rotation and oversized-event tests |
...
| `observe.BuildReport(directory, options)` | `internal/observe` | Managed schema-1/2 JSONL and explicit lower time bound | Report v2 identity-free readiness/path/Guard/session plus post-commit Direct/Proxy bytes, termination and duration | Relay events require schema 2; rejects corruption/overflow; returns no identity and never infers application success or total savings | experimental | Mixed-version/path aggregation, overflow, cutoff, privacy, corruption and source-binding tests |
...
| `sidecar.Server.Serve(ctx, listener)` | `internal/sidecar` | Context, listener, racers, optional clock and outcome callback | Cancels/joins handlers; TLS requires L3; emits one bounded outcome after committed relay termination | Rejects unsafe ClientHello; outcome contains counters only and does not feed learning; never reads Clash config | experimental | Sidecar lifecycle/readiness/byte-count tests, Test Lab and Mihomo Lab |
| `Server.OnRelayOutcome(event)` | `internal/sidecar` | Target, selected path, post-commit directional bytes, duration and bounded termination | One callback after both relay directions end | Optional; no payload/raw error/application status; callback is absent in isolated labs unless explicitly tested | experimental | Exact byte/duration/path and privacy persistence tests |
...
| `netrelay.Bidirectional(ctx, left, right)` | `internal/netrelay` | Context and two owned TCP-like connections | Directional copied-byte counts plus explicit cancellation; closes/joins both directions | Counts post-call bytes only; ignores raw copy errors; results are not routing/application evidence | experimental | Exact payload count and two-end cancellation test; Guard/Sidecar consumers |
...
| `decision` | Sidecar/decision engine | `event_type`, target, selected path, reason, optional privacy/learning/durable reasons and ephemeral policy state, winner `observation`, optional completed `other_observation`, `committed`, optional `decision_latency_ms` | CLI/recorder/report/UI | experimental `DecisionEvent`; successful TLS latency starts after safe ClientHello/privacy decision and includes stagger/fallback; schema-1 rows remain readable and new rows use schema 2 |
| `relay_outcome` | Sidecar after relay | HMAC-transformed target in recorder, selected path, directional post-commit byte counts, duration, `ended`/`canceled` | Identity-free trial report | experimental schema-2 event; no payload/raw error; remote bytes may be TLS readiness and are not application success; never enters learning |
...
| `guard_decision` | Guard | `event_type`, target, selected lane, reason, bounded failure classes, `committed` | CLI/recorder/UI | experimental; schema-1 rows remain readable and new rows use schema 2; no payload bytes |
