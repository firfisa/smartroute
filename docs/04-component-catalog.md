...
| `internal/sidecar` | Data plane | experimental | SOCKS admission, TLS first-flight orchestration, stage enforcement, relay, decision/diagnostic events, and cancellation-time handler draining | SQLite I/O, durable policy evaluation, post-commit replay, or active Clash modification | Sidecar net.Pipe lifecycle tests, Test Lab and Mihomo Lab |
...
| `internal/netrelay` | Data plane | experimental | Context-aware bidirectional TCP copying, half-close handling, cancellation closure, and copy-goroutine join | Routing decisions, retry, replay, or payload inspection | `internal/netrelay/relay_test.go`; consumers in Guard, Sidecar and Test Lab |
...
| `sidecar.Server.Serve(ctx, listener)` | `internal/sidecar` | Context, listener, plain Racer or TLSRacer, optional injected clock | Serves until cancellation/error; cancels and joins every accepted handler before return; TLS mode requires L3 | Rejects unsafe ClientHello before dialing; shutdown interrupts rather than replays; never reads Clash config | experimental | Sidecar lifecycle/readiness tests, Test Lab and Mihomo Lab |
...
| `netrelay.Bidirectional(ctx, left, right)` | `internal/netrelay` | Context and two owned TCP-like connections | Relays both directions; context cancellation closes both endpoints and joins copy goroutines | Best-effort half-close; relay/cancellation errors are not routing evidence | experimental | Payload transparency and two-end cancellation test; Guard/Sidecar consumers |
