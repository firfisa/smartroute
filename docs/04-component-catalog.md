...
| `sidecar.Server.Serve(ctx, listener)` | `internal/sidecar` | Context, listener, plain Racer or TLSRacer, optional injected clock | Serves until cancellation/error; TLS mode requires L3; successful TLS decisions include post-ClientHello decision-to-readiness milliseconds | Rejects unsafe ClientHello before dialing; never reads Clash config | experimental | Sidecar, deterministic latency, Test Lab and Mihomo Lab |
