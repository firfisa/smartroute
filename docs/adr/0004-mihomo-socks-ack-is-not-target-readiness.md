...
5. Mihomo adaptive routing remains gated until a protocol-aware readiness layer proves target readiness. HTTPS/TLS ClientHello and valid server TLS records are the next supported path.
...
- The current experimental `serve` command fails safely with Mihomo candidate endpoints instead of allowing a false-positive connection.
...
- Isolated runtime: `make mihomo-lab` verifies Direct, forced Proxy, domain preservation, loop prevention, and the rejected below-commit-stage scenario.
